...
const showMeLastFlights = () => {
    ...
    for (let vuelo = flights.length -6; vuelo <flights.length - 1; vuelo++){
...
const deleteFlight = (flights) => {
    const flightIds = Object.values(flights).map(flight => flight.id)
    console.log(`Id's de vuelos activos:\n${flightIds}`)
    ...
    console.log(`\nVuelo borrado:`)
    console.log(flights[flightIds.indexOf(idToDelete)])
    ...
    switch (true) {        
        ...
        case (priceMax > 0):
            ...
            for (let i = 0; i < indexUnderPriceFlights.length; i++){
                console.log(flights[indexUnderPriceFlights[i]])
                }
            }            
    }
