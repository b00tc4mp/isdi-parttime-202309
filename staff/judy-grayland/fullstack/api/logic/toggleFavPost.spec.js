      ...
      return Promise.all([
        ...
        return toggleFavPost(user1.id, post3.id)
          .then((value) => {
            ...
            return User.findById(user1.id).then((user1) => {
              const postIdExists = user1.favs.some(
                (postObjectId) => postObjectId.toString() === post3.id
              )
          ...
          .then(() => {
            return toggleFavPost(user1.id, post3.id).then((value) => {
              expect(value).to.be.undefined
