  ...
  return (
    <>
       ...
       {logic.context.token && logic.context.isAdmin && (<button className='button-add text-gray-600 transition-colors duration-300 cursor-pointer' onClick={handleNewHistoryClick}><AiFillFileAdd size={20} /></button>)}
        ...
        {
          history.lenght !== 0 && (history.map((history) => {
            return <div key={history.id}>
            <div className="max-w-sm bg-gray-900 border border-gray-200 rounded-lg shadow-custom text-gray-100 dark:border-yellow-800">
                    <img className="rounded-t-lg" src={history.image} alt="" />
                <div className="p-5">
