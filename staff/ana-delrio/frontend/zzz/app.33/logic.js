class Logic {
    ...
    registerUser(name, email, password, callback) {
        ...
        db.users.findByEmail(email, (error, user) => {
            // este if maneja errores durante la búsqueda del usuario en la base de datos, 
            if (error) {
                callback(error)

                return
            }
            // este if maneja el caso en el que se encuentra un usuario con el mismo correo electrónico que el que se está utilizando 
            if (user) {
                callback(new Error('user already exists'))

                return
            }
            // este insert me podría lanzar un error
            db.users.insert(new User(null, name, email, password, []), error => {
                ...
                // aquí llegaríamos si todo ha ido bien
                callback(null)
            })
        })
    }
    ...
    loginUser(email, password, callback) {
        ...
        db.users.findByEmail(email, (error, user) => {
            ...
            // no hubo errores, por eso ponemos null
            callback(null)
        })
    ...
    retrieveUser(callback) {
        db.users.findById(this.sessionUserId, (error, user) => {
            if (error) {
                // llamamos al callback de arriba
                callback(error)
            ...
            // no hubo error, y después, te envío el uduario
            callback(null, user)
        })
    }
