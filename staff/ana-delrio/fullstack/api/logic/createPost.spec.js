...
describe('createPost', () => {
            ...
            .then(user => {
                return createPost(user.id, image, text)
                    .then(value => {
                        ...
                        // we check that nothing is returned: undefined
                        expect(value).to.be.undefined

                        // we check in the dddb if the post exists
                        return Post.findOne({ author: user.id })
                    })
                    .then(post => {
                        // we check that the post has the above image and text
    ...
    it('fails on-non already existing user', () => {
        const image = random.image()
        const text = random.text()
